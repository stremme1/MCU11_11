...
void configurePLL() {
    ...
    DEBUG_PRINTLN("  Configuring PLL...");
    
    // Turn off PLL
    RCC->CR &= ~(1 << 24);
    DEBUG_PRINTLN("  PLL turned off");
    
    // Wait till PLL is unlocked (e.g., off)
    DEBUG_PRINTLN("  Waiting for PLL to unlock...");
    int timeout = 10000;
    while ((RCC->CR >> 25 & 1) != 0 && timeout-- > 0);
    if (timeout <= 0) {
        DEBUG_PRINTLN("  ERROR: PLL unlock timeout!");
        return;
    }
    DEBUG_PRINTLN("  PLL unlocked");
    ...
    // Enable PLL
    RCC->CR |= (1 << 24);
    DEBUG_PRINTLN("  PLL enabled, waiting for lock...");
    
    // Wait until PLL is locked
    timeout = 10000;
    while ((RCC->CR >> 25 & 1) != 1 && timeout-- > 0);
    if (timeout <= 0) {
        DEBUG_PRINTLN("  ERROR: PLL lock timeout!");
        return;
    }
    DEBUG_PRINTLN("  PLL locked");
}

void configureClock(){
    DEBUG_PRINTLN("  Setting prescalers...");
    ...
    DEBUG_PRINTLN("  Selecting PLL as clock source...");
    ...
    DEBUG_PRINTLN("  Waiting for clock switch...");
    int timeout = 10000;
    while(!((RCC->CFGR >> 2) & 0b11) && timeout-- > 0);
    if (timeout <= 0) {
        DEBUG_PRINTLN("  ERROR: Clock switch timeout!");
        return;
    }
    DEBUG_PRINTLN("  Clock switched to PLL");
